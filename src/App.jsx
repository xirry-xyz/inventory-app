...
export default function App() {
  ...
  const [newItem, setNewItem] = useState({
    name: '',
    category: 'daily',
    quantity: 1, // 确保初始状态是数字
    unit: '个',
    minQuantity: 1, // 确保初始状态是数字
    note: ''
  });
  ...
  const handleAddItem = (e) => {
    ...
    // 状态中的 quantity 和 minQuantity 现在已经是数字，可以直接进行验证
    if (!newItem.name.trim() || newItem.quantity < 0 || newItem.minQuantity < 0) return;

    const item = {
      id: Date.now(),
      ...newItem,
      // 移除冗余的 Number() 转换，因为状态在 onChange 时已转换为数字
    };
    setItems([item, ...items]);
  ...
  const updateQuantity = (id, delta) => {
    setItems(items.map(item => {
      if (item.id === id) {
        // item.quantity 已经是数字
        const newQty = Math.max(0, item.quantity + delta); // 确保数量不小于0
